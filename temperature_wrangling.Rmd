...
Save as a CSV--remove the pound signs if you want to re-create the CSV
...
First look at how the temperature changes over time

```{r}
temp_time_avgs %>% 
  filter(Date_time > as_datetime("2020-02-01 00:00:00")) %>% 
  ggplot(aes(x = Date_time, y = Mean_temp)) +
  geom_line() +
  geom_line(aes(x = as_datetime(Day), y = Mean_max_C_4), data = subset(all_temp, Day > as_datetime("2020-01-31")), color = "red") +
  geom_hline(yintercept = 27.3, color = "blue", linetype = "dashed") + # this is the Heron monthly mean water temp according to: https://seatemperature.info/heron-island-water-temperature.html and also Jesse + Gus
  theme_classic()
```
