...
Start with the full model with all terms and their interactions
...
Then use a stepwise selection process to pick the most parsimonious model

```{r}
mGaus.step <- step(mGaus, direction = "both")
  summary(mGaus.step) 
...
pred$PredTissueLoss <- predict(mGaus.step, pred, type = "response") 
...
# make it match the panel above
pred %>% 
  mutate(Crab_treatment = factor(Crab_treatment, levels = c("N", "Y"))) -> pred
...
pred %>% 
  ...
  xlab("Mean maximum temperature-4 day (°C)") +
  scale_color_manual(values = c(darkO, skyB),
                     name = "Algae") + # clean up legend and colors
  theme_classic() +
  theme(legend.position = "none") -> modelfig
...
All terms *except*:
- Wounding 
- Wounding:temperature
- Wounding:crab
- Wounding:crab:temperature

**Observations from the plot**
See that: 
- Temperature had the largest effect on tissue loss when there was no crab, algae, and no wounding
- Temperature had very little effect on tissue loss when there was a crab present, although if there was algae this relationship steepened (looks about parallel with no algae/no crab)
- Algae increased the amount of tissue lost
- Wounding appears to decrease the strength of the relationship between temperature and crabs/algae/tissue loss
...
(tempTSFig + maxTSFig) / (tissueLFig) / (modelfig)
